...
public class AuthTest extends BaseTest {
        ...
        /**
         * Sends credentials payload, validates status code 200, and extracts the
         * password
         ...
         */
        @Test(dependsOnMethods = "testLoginAndExtractToken", groups = {
                        "auth" }, description = "Demonstrate passing Bearer token to a secured endpoint")
        public void testSecuredRouteWithBearerToken() {
                ...
                // Verify authentication success. Support both String value (Python httpbin) and
                // Array/List (Go httpbin)
                response.then()
                                .statusCode(200)
        ...
        /**
         * Executes GET request to /headers supplying a Bearer authentication token
         * using a pre-configured spec.
         * Asserts that the server processes and returns the header correctly.
         */
        @Test(dependsOnMethods = "testLoginAndExtractToken", groups = {
                        "auth" }, description = "Demonstrate passing Bearer token using a pre-configured RequestSpecification")
        public void testSecuredRouteWithTokenSpec() {
                log.info("Sending Bearer token using getHttpBinSpecWithToken spec wrapper to /headers");

                Response response = given()
                                .spec(BaseAPI.getHttpBinSpecWithToken(extractedToken))
                                .when()
                                .get("/headers");
        ...
        public void testStandardBasicAuth() {
                ...
                // Support both "authenticated: true" (Python/recent Go httpbin) and
                // "authorized: true" (older Go httpbin)
        ...
        public void testPreemptiveBasicAuth() {
                // Preemptive auth sends credentials directly in request header without waiting
                // for 401 challenge
                Response response = given()
                                .spec(BaseAPI.getHttpBinSpec())
                                .auth()
